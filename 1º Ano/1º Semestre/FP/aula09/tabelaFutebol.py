...
def main():
    ...
    tab = sorted(tabela, key=lambda t: pontos(t),reverse=True)
    printTabela(tab)

    # c)
    # Acrescente os argumentos adequados à função sorted para
    # obter uma tabela ordenada por ordem decrescente da diferença GM-GS:
    tab = sorted(tabela, key=lambda t: t[4]-t[5], reverse=True )
    printTabela(tab)
    ...
    tab = sorted(tabela, key=lambda t: (pontos(t), t[4]-t[5]), reverse=True)
    printTabela(tab)
